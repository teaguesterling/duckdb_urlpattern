...
public:
	...
	explicit Re2RegexWrapper(std::shared_ptr<duckdb_re2::Regex> regex) : regex_(std::move(regex)) {
	}
...
public:
	...
	static std::optional<std::vector<std::optional<std::string>>> regex_search(std::string_view input,
